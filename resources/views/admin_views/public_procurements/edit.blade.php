...
<form action="{{ route('public.procurements.update', ['id' => $publicProcurement->id]) }}" method="post">
    @csrf
    @method('PUT')
    <label for="">Име</label>
    <input type="text" name='name' value="{{$publicProcurement->name}}">
    <label for="">Линк</label>
    <input type="text" name='link' value="{{$publicProcurement->link}}">
